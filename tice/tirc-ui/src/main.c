/*
 *--------------------------------------
 * Program Name: tirc-ui
 * Author: Wowkster
 * License: MIT
 * Description: Allows the user to interact with a TI-RC chat server
 *--------------------------------------
 */
...
static usb_error_t handle_usb_event(usb_event_t event, void *event_data, usb_callback_data_t *callback_data)
{
    usb_error_t err;
...
int main(void)
{
    ...
    do
    {
        ...
        if (has_srl_device)
        {
            ...
            {
                char chars[65];
                memcpy(chars, in_buf, bytes_read);
                chars[64] = '\0';
                printf("Got data: %s\n", chars);
